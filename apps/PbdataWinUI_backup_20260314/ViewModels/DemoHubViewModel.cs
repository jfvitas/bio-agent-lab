...
public sealed partial class DemoHubViewModel : BaseViewModel
{
    ...
    public DemoHubViewModel()
    {
        ...
        {
            new() { Key = "tri_modal", Label = "Structure + sequence + assay", Caption = "Best for realistic, cross-source training demos." },
        ...
        {
            new() { Key = "fast", Label = "Fast demo", Caption = "Shortest simulated train time and lighter output." },
        ...
        {
            new() { Number = 1, PageKey = "Guide", Title = "Frame the story", ActionLabel = "Start guided demo", WhyItMatters = "This primes the workspace with a coherent scientific story instead of a pile of controls.", WhatToClick = "Start guided demo", HowToFindIt = "Open Guided Demo and use the first primary button." },
            new() { Number = 2, PageKey = "Dataset", Title = "Preview representative coverage", ActionLabel = "Preview search", WhyItMatters = "The platform shows broad structural coverage rather than returning a narrow cluster of near-duplicates.", WhatToClick = "Preview Search", HowToFindIt = "Open Dataset Builder and use the first action button." },
            new() { Number = 3, PageKey = "Dataset", Title = "Assemble the graph-ready set", ActionLabel = "Assemble set", WhyItMatters = "This reconciles structure, assay, sequence, and motif context into a balanced demo dataset.", WhatToClick = "Assemble Set", HowToFindIt = "Stay in Dataset Builder and use the middle action button." },
            new() { Number = 4, PageKey = "Dataset", Title = "Design the leakage-resistant split", ActionLabel = "Design split", WhyItMatters = "The split logic emphasizes family holdout, motif grouping, and source-aware evaluation.", WhatToClick = "Design Split", HowToFindIt = "Use the third Dataset Builder action button." },
            new() { Number = 5, PageKey = "Model", Title = "Refresh the model path", ActionLabel = "Recommend", WhyItMatters = "Model Studio explains why a given architecture suits the selected modalities and objective.", WhatToClick = "Recommend", HowToFindIt = "Open Model Studio and use the first action button." },
            new() { Number = 6, PageKey = "Model", Title = "Train the candidate", ActionLabel = "Train", WhyItMatters = "The training view shows realistic curves, scores, and architecture tradeoffs.", WhatToClick = "Train", HowToFindIt = "Use the primary training button in Model Studio." },
    ...
    public string DemoHeadline => "A polished, fully simulated walkthrough of how pbdata turns broad structural evidence into balanced ML-ready datasets and explainable model outputs.";
    public string DemoDisclaimer => "Demo Mode is active. Every chart, score, and artifact in this shell is simulated to show intended behavior without long-running ingestion or training.";
    ...
    {
        ResetDemo();
        AppendLog("Guide", "Guided walkthrough restarted with a broad, representative discovery scenario.");
        AdvanceToStep(1);
    }
    ...
    private void ResetDemo()
    {
        ...
        ActivityLog.Clear();
        AppendLog("Demo", "Demo workspace seeded with a representative multi-source protein-binding story.");

        RefreshDatasetProjection();
        RefreshModelProjection();
        RefreshInferenceProjection();
        RebuildTimeline();
        RebuildArtifacts("Ready to simulate");
        NotifyGuideState();
    }
    ...
    {
        AppendLog("Search", "Simulated representative search expanded across protein-protein, protein-ligand, and single-protein control strata.");
        AdvanceToStep(2);
    }
    ...
    private void AssembleDataset()
    {
        ...
        AdvanceToStep(3);
        RebuildArtifacts("Dataset package simulated");
    }
    ...
    private void DesignSplit()
    {
        ...
        AdvanceToStep(4);
        RebuildArtifacts("Split manifest simulated");
    }
    ...
    private void TrainModel()
    {
        ...
        RunSummary = SelectedModelFamilyKey switch
        {
            ...
            _ => "Fusion model that blends structure graphs, embeddings, and assay descriptors to produce the strongest overall demo performance.",
        };

        AppendLog("Training", $"Trained {SelectedRunName} and produced realistic chart outputs, scorecards, and saved artifacts.");
        AdvanceToStep(6);
        RebuildArtifacts("Training report simulated");
    ...
    private void CompareRuns()
    {
        ...
        AdvanceToStep(6);
        RebuildArtifacts("Comparison report simulated");
    }
    ...
    private void RunInference()
    {
        ...
        RebuildArtifacts("Inference packet simulated");
    }
    ...
    private void RefreshDatasetProjection()
    {
        ...
        DatasetStats.Add(new() { Label = "Candidate structures", Value = $"{profile.candidates:N0}", Caption = "Representative structures that match the selected demo scope." });
    ...
    private void RefreshModelProjection()
    {
        ...
        TrainingStats.Add(new() { Label = "Calibration", Value = (0.79 + (baseAuprc - 0.70) * 0.65).ToString("0.000"), Caption = "Confidence quality after the simulated post-hoc calibration step." });
        TrainingStats.Add(new() { Label = "Train time", Value = speed switch { "fast" => "3m 12s", "best" => "11m 48s", _ => "6m 24s" }, Caption = "Shortened for demo mode while preserving realistic differences." });
    ...
    private void RefreshInferenceProjection()
    {
        ...
        else if (SelectedInferenceScenarioKey == "mutant_scan")
        {
            ...
            InferenceNarrative = "Inference now reads like a mutation sensitivity pass, where the demo surfaces how motif-preserving and motif-disrupting residue changes alter score confidence.";
        }
        else
        {
            Predictions.Add(new() { PairLabel = "MCL1 hydrophobic cleft", Score = "0.921", Confidence = "High", Rationale = "Pocket geometry, motif coverage, and embedding context all align with strong binder patterns.", RiskNote = "Minimal; chemistry is well represented in the simulated training set." });
    ...
    private void RebuildComparisonRuns()
    {
        ...
        ComparisonRuns.Add(new() { Name = "Hybrid Fusion - novelty holdout", Family = "Hybrid Fusion", HeadlineMetric = "AUPRC 0.846", SupportMetric = "AUROC 0.921", Note = "Best overall transfer in the simulated demo run set.", AccentBrush = DemoPalette.Aqua });
